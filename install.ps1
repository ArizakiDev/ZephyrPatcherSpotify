param (
  [Parameter()]
  [switch]
  $UninstallSpotifyStoreEdition = (Read-Host -Prompt "Désinstallez l'edition Spotify Windows Store si elle existe (O/N)") -eq 'y',
  [Parameter()]
  [switch]
  $UpdateSpotify
)
...
{
  Write-Host "La version Microsoft Store de Spotify a été detectee et n'est pas prise en charge.`n"
...
{
  Write-Host 'Telechargement de la dernière version de spotify, merci de patienter...'
  ...
  if ($isUserAdmin)
  {
    ...
    Write-Host "La tâche d'installation a été planifiée. Démarrage de la tâche..."
